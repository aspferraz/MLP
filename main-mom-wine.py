...
initialWeightsArr = np.load('wine-weights/ini_weights_13-12-3.npy', allow_pickle=True)
...
for i in range(10):
    ...
    X_t = scaler.transform(dfs[i][1].iloc[:, 0:13])
    # X_t = dfs[i][1].iloc[:, 0:13].values
    y_t = dfs[i][1].iloc[:, [14, 15, 16]].values
    ...
    for j in range(0, 3):
        ...
        if (i == 0):
            ...
            ax.set_title('Wine exec: %d for fold %d' % (j+1,i+1))
...
df.to_csv('wine-mom-executions/executions_%s.csv' % topologyDesc, sep = ';')
df.describe().to_csv('wine-mom-executions/executions_described_%s.csv' % topologyDesc, sep = ';')
